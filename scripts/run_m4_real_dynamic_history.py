#!/usr/bin/env python3
"""Run the local-only measured M4.8 real dynamic history."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run a measured real-model INSERT/DELETE/REPLACE M4 history, "
            "dual-oracle audits, and reconnecting zero-call exact replays."
        )
    )
    parser.add_argument(
        "--database-url",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M4.8 real dynamic history unavailable: {error}", file=sys.stderr)
        return 2
    if args.output is not None:
        write_dynamic_history_manifest(result, args.output)

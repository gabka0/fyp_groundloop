...
- `docs/workstreams/m5_runtime_contract/REQUIREMENT_STATE_COUNTER_ERRATUM.md`
  -- accepted M5-D27 requirement-state counter-ownership correction;
- `docs/workstreams/m5_runtime_contract/PHASED_PERSISTED_MATCHING_COMPOSITION_AMENDMENT.md`
  -- accepted M5-D28 phased first-application and retained-replay composition
  correction;
- `docs/workstreams/m5_runtime_contract/BOUNDED_DOCUMENT_WITHDRAWAL_AMENDMENT.md`
  -- accepted M5-D29 persisted legacy-source, bounded withdrawal/declaration,
...
### 4.4 Accepted M5-D27 requirement-state counter-ownership erratum

The separately reviewed D27 candidate at commit `bbfd4bf`, tree
`4a2f2cc72c0e46410a76b3a80078130c194b2bd7`, and SHA-256
`7a51afc1f1b6c249572222a023814de8b22220058e00f09564de64f552bd411c`
is accepted as the authoritative wording-only correction. The first candidate
was rejected with three P1 findings and corrected. The decisive independent
authority/semantics and PostgreSQL/schema/compatibility audits then reviewed
the same final bytes and each returned `GO`, `P0=0`, `P1=0`.

This closes only the D27 contract gate and advances the runtime addendum to
revision 8. M5.0-27 is contract-`PASS` / implementation-`PENDING`. D24 has no
dedicated requirement-state physical row-count coordinate, and no D24 or D25
counter aliases it. Requirement-state mutations remain exact D25 logical
patch/output and transition-bijection evidence. Any transaction-local planned/
actual count is diagnostic only and is neither persisted nor hashed.

No DTO, digest, migration, schema, source, test, database, provider,
deployment, runtime activation, measured result, or AI-quality result is
implemented. M5-D24 through M5-D27 and M5.0-24 through M5.0-27 remain
...
### 4.5 Accepted M5-D28 phased persisted-matching composition contract

The separately reviewed D28 candidate at commit
`cc0b9cf25110992320dc0b7f81499a3930d1a565`, tree
`760107f9b6cbd70d37839144980cf23b8f5859ea`, and SHA-256
`8a2bafd3478cf2cac6ac7c8de7ca7779a6d9ace7fbbe98a7dc3ff08afdb67eae`
is accepted as the authoritative narrow sequencing and replay correction. The
candidate was refined through repeated exact-byte audits. The decisive
authority/semantics and direct-15c/PostgreSQL reviewers then audited the same
810-line, 48,053-byte commit and each returned `GO`, `P0=0`, `P1=0`, `P2=0`.

This closes only the D28 contract gate and advances the runtime addendum to
revision 9. M5.0-28 is contract-`PASS` / implementation-`PENDING`. First
application now has an explicit cursor-bound prepare/header/stage/D24/finalize
composition; direct sources use a complete pre-15c reservation and lexical M4
stage evidence; and historical replay validates a canonical retained changed-
key intent projection while returning the one current retained accumulator.
The correction changes relative DML placement only where accepted migration
guards require it. It changes no public signature, DTO, digest recipe, counter,
schema, migration, source/reference kind, M4-v1 bytes, or runtime mode.

No source, test, migration, database, provider, deployment, runtime activation,
measured result, or AI-quality result is implemented. The held D27 continuation
draft and the original unstarted Task-2 C1/C2/C3/D grants are not authority. A
new separately reviewed path-exclusive activation must start from the exact
pushed D28 authority barrier. M5-D24 through M5-D28 and M5.0-24 through
M5.0-28 remain implementation-`PENDING`; Task 2, M5.4-05 through M5.4-09, and
every M5.5/M5.6 gate remain `PENDING`; runtime remains `v1_only` outside
isolated fixtures.

### 4.6 Accepted M5-D29 bounded document-withdrawal contract

The separately reviewed D29 candidate at commit
`4667da0e230d2b52b3147d0ee771396b406ed1e9`, tree
`5f7e69e6f2f503fc3a36691fee16afa40bc8a479`, and SHA-256
`e05f159f98d5f282335a90d2e9db1a26f8d85560030314060d918df59ccc82fb`
is accepted as the authoritative narrow persisted-source, bounded-withdrawal,
declaration-completeness, same-policy, and terminal-cutoff correction. Three
independent same-byte candidate audits returned `GO`, `P0=0`, `P1=0`, `P2=0`;
two decisive reviewers then independently confirmed the exact 1,322-line,
81,878-byte commit provenance and unchanged verdict.

At that D29 checkpoint, this closed only the contract gate and advanced the
runtime addendum to revision 10. M5.0-29 was contract-`PASS` /
implementation-`PENDING`. The locked
epoch event ID and payload hash are persisted legacy source identity, while
all sidecars remain independent exact authority. Exact bounded locators and
the one-top-level-key typed manifest commitment close reverse/current and
direct/M5 declaration enumeration. Same-policy fail-closed withdrawal is the
sole semantic-admissibility change; cross-policy and typed rootless
`ObserveRequirementEvent` history remain `PENDING`. Activation-bootstrap
observations remain supported, and already-terminal D24-valid unresolved
dispatch/attempt history is preserved for late audit rather than recancelled.
Pre-opener terminal races use ordinary canonical replay; the post-open private
cutoff uses the existing checked active projection with exact zero call work.

Migration 018 was then only contract-authorized with exactly two locator
indexes and one atomic ledger-first install/barrier protocol; no migration or
runtime bytes had yet been accepted, and a new D29 activation was still
required. That activation, migration 018, and application routing later
integrated at `8ed44a6`, `9c855b5`, and `8734162` as partial historical
evidence. They did not close Task 2 or any decision-row implementation half;
held Lane-P bytes remained read-only non-authority. M5-D24 through M5-D29 and
M5.0-24 through M5.0-29 remained implementation-`PENDING`; Task 2, M5.4-05
through M5.4-09, and every M5.5/M5.6 gate remained `PENDING`; runtime remained
`v1_only` outside isolated fixtures.

### 4.7 Accepted M5-D30 claim-current provenance contract

The separately reviewed D30 candidate at commit
`36998d1acf4d3f4248b7d1fdcad7ff939272dd21`, tree
`048d55b49647e641735cb9c6a738c5a3c4a7290b`, sole parent
`8734162f8578ac3105119789a8729fc662da575d`, and SHA-256
`db2568affc02cf1ca6f17a549029f31089cecd857debdedf2651e6aac6898fe4`
is accepted as the authoritative narrow total claim-current and direct-M4/M3
provenance correction. Two independent same-byte candidate audits returned
`GO`, `P0=0`, `P1=0`; both then confirmed the exact 647-line, 35,791-byte
commit provenance and unchanged verdict.

This closes only the D30 contract gate and advances the runtime addendum to
revision 11. M5.0-30 is contract-`PASS` / implementation-`PENDING`. One total
changed-chunk current-currency range now covers both subject kinds. Dynamic
claims are authorized by exact working-delta, child/admission, complete owner,
optional execution and currency closure; activation-base claims are authorized
by exact M3 epoch/run/execution/candidate/artifact closure. The predecessor
interval route is physically one-row bounded. No unavailable root membership,
reason hit, sibling source, classic artifact, pair input, judgment, or
aggregate preimage is reconstructed.

D30 adds no schema or migration; migration 018 remains exact and no migration
019 exists. The integrated D29 activation at `8ed44a6`, migration-018 tranche
at `9c855b5`, and application-routing tranche at `8734162` are retained partial
historical evidence, not D30 implementation acceptance. The 15-path Lane-P WIP
is read-only non-authority until a new path-exclusive activation is audited,
integrated, and pushed from the exact D30 barrier. No source, test, database,
provider, deployment, runtime activation, measured result, or AI/model-quality
result is accepted. M5-D24 through M5-D30 and M5.0-24 through M5.0-30 remain
implementation-`PENDING`; Task 2, M5.4-05 through M5.4-09, and every M5.5/M5.6
gate remain `PENDING`; runtime remains `v1_only` outside isolated fixtures.

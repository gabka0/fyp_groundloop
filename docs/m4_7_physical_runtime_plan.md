...
Status: active corrective stage; blocks any end-to-end event-time complexity
claim and blocks M5 implementation.
...
M4.7 preserves the sparse SQL representation and replaces the hidden global
work. Until every gate below passes, the only allowed performance statement is
"sparse PostgreSQL overlay/publication writes were observed." The stronger
term "physically incremental M4 runtime" is forbidden.
...
## 3. Corrected parameters and target bound
...
The defensible structured-work target is output sensitive:
...
Expected hash-map assumptions, ordered-index logarithmic factors, PostgreSQL
index/I/O costs and `O(B)` hashing are stated beside any result. Dense fanout
remains linear in the enumerated stored edges. No sublinear worst-case claim is
possible for dense deletion, large admitted output, or large published state
change.

This corrects the M4 freeze formula, which omitted unavoidable `P+`, `H` and
attempt work. The correction is an explicit design amendment; it is not a
silent rewrite of the frozen document.

## 4. Work packages

### M4.7a — policy-build claim-registry snapshots
...
### M4.7b — point-CAS runtime coordination
...
### M4.7c — signed compact-evaluation counters
...
### M4.7d — affected-key direct-witness patches
...
### M4.7e — integration and adversarial scaling gate
...
M4.7 alone does not complete M4.
...
M5 bounded evidence groups begins only after M4.7, M4.8 and M4.9 pass and the
M4 status document records exact commands, manifests and limitations.

## 6. Parallel ownership

The active independent lanes are:
...
Agents commit only their exclusive paths. The coordinator reviews and
integrates one lane at a time, resolves migrations centrally, then runs the
combined live gate. Passing lane-local tests is not M4.7 acceptance.
